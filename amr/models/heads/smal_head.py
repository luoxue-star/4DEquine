...
class SMALTransformerDecoderHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.decshape = nn.Linear(dim, cfg.SMAL.SHAPE_DIM)
        ...
        init_betas = torch.zeros(size=(1, cfg.SMAL.SHAPE_DIM), dtype=torch.float32)
    ...
    def forward(self, x, **kwargs):
        ...
        pred_pose_matrix = joint_conversion_fn(pred_pose).view(batch_size, self.cfg.SMAL.NUM_JOINTS + 1, 3, 3)

        pred_smal_params = {'global_orient': pred_pose_matrix[:, [0]],
                            'pose': pred_pose_matrix[:, 1:],
                            'betas': pred_betas,
                            }
        return pred_smal_params, pred_cam, pred_pose.view(batch_size, -1)
