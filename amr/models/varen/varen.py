...
VERTS2KPTS =    [[798, 881, 904], [2714, 2798, 2821],  # left and right ear
                [1250], [3498],  # left and right eye
                [1739],  # nose
                [4763, 4768], # neck
                [1717], [1915], [1899],  # tail start, tail tip, half tail
                [1472, 1481, 1476, 1473], [3389, 3398, 3393, 3390],  # left and right front foot
                [4135, 4215], [4136, 2318], 
                [3863, 3856, 3984], [3864, 3859, 3987],  # left and right front shoulder
                [1410, 1412, 1405, 1409], [3327, 3329, 3322, 3326],  # left and right back foot
                [12803, 5429], [12804, 5430],  # left and right back knee
                [6195], [6196]  # left and right back knee
                ] 
...
class VAREN(HSMAL):
    ...
    def freeze_muscle_deformer(self):
        if self.use_muscle_deformations:
            for param in self.betas_muscle_predictor.parameters():
                param.requires_grad = False
            
            for module in self.Bm:
                if module is not None:
                    for param in module.parameters():
                        param.requires_grad = False
            
            self.muscle_deformer_frozen = True
    ...
    def forward(self,
        ...
        transl = torch.zeros(size=(betas.shape[0], 3), device=betas.device, dtype=betas.dtype) if transl is None else transl
        ...
        keypoint = []
        for kp_v in VERTS2KPTS:
            keypoint.append(vertices[:, kp_v, :].mean(dim=1))
        extra_joints = torch.stack(keypoint, dim=1)
