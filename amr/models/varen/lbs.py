...
def lbs(  # noqa: PLR0913, PLR0914, PLR0917
    ...
    J_transformed, A = batch_rigid_transform(
        rot_mats, J, parents, tail_scale=tail_scale, dtype=dtype
    )
...
def batch_rigid_transform(
    ...
    joints: Tensor,
    parents: Tensor,
    tail_scale: Tensor = None,
    tail_joints: list = list(range(30, 35)),
    dtype=torch.float32
) -> Tensor:
    """Apply a batch of rigid transformations to the joints with optional tail scaling.
    Parameters
    ----------
    ...
    tail_scale : torch.tensor Bx2, optional
        Scaling parameters for tail joints [lengthening, fatness]
        If None, no scaling is applied
    tail_joints : list, optional
        List of joint indices that belong to the tail (e.g., [25, 26, 27, 28, 29, 30, 31])
        If None, defaults to range(25, 32)
    ...
    rel_transforms : torch.tensor BxNx4x4
        ...
        for all the joints
    """
    B, N = joints.shape[0], joints.shape[1]
    
    tail_scale = tail_scale if tail_scale is not None else torch.zeros(B, 2).to(joints.device)
    # Initialize scaling factors
    scaling_factors = torch.ones(B, N, 3).to(joints.device)
    # tail_scale is Bx2: [lengthening, fatness]
    # Apply lengthening to x-axis, fatness to y,z-axes for tail joints
    tail_scale_exp = torch.exp(tail_scale)  # Ensure positive scaling
    for joint_idx in tail_joints:
        if joint_idx < N:  # Ensure joint index is valid
            scaling_factors[:, joint_idx, 0] = tail_scale_exp[:, 0]  # x-axis lengthening
            scaling_factors[:, joint_idx, 1] = tail_scale_exp[:, 1]  # y-axis fatness
            scaling_factors[:, joint_idx, 2] = tail_scale_exp[:, 1]  # z-axis fatness
    
    # Create 3x3 scaling matrices
    ...
    # Apply scaling to rotation matrices
    rot_mats_scaled = rot_mats.clone()
    for i in range(1, parents.shape[0]):
        parent_idx = parents[i]
        # Apply scaling: s_parent_inv @ R @ s_current
        s_par_inv = torch.inverse(scale_factors_3x3[:, parent_idx])
        s_curr = scale_factors_3x3[:, i]
        rot_mats_scaled[:, i] = s_par_inv @ rot_mats[:, i] @ s_curr
    
    transforms_mat = transform_mat(
        rot_mats_scaled.reshape(-1, 3, 3),
...
def muscle_deformations(vertices, muscle_deformer):
    ...
    for i in range(len(Bm)):
        if Bm[i] is not None:  # Handle Muscle with no Vertices
            idx = muscle_idxs[i]
            mdv[:, idx] = Bm[i].forward(betas_muscle[:, i]).view(vertices.shape[0], -1, 3).float()
