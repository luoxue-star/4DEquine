...
class Renderer:
    ...
    def __call__(self,
        ...
        color = color.astype(np.float32) / 255.0 if color.shape[-1] == 4 else np.concatenate([color, np.where(rend_depth > 0, 255, 0)[:, :, np.newaxis]], axis=-1) / 255.0
        renderer.delete()
    ...
    def render_rgba_blender(
        ...
        # Rotate Mesh from Blender Camera Coordinate to PyRender Coordinate
        vertex_colors = np.array([(*mesh_base_color, 1.0)] * vertices.shape[0])
