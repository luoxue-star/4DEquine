...
def render_keypoint(img: np.array, keypoint: np.array, threshold=0.1,
    ...
    elif keypoint.shape[0] == 21:
        pairs = [0, 2,  1, 3,  2, 4,  3, 4,  4, 5,  5, 6,  6, 8,  7, 8,  9, 11,  10, 12,
                 11, 13,  12, 14,  13, 5,  14, 5,  15, 17,  16, 18,  
                 17, 19,  18, 20,  19, 6,  20, 6]
    else:
...
class SilhouetteRenderer:
    ...
    def __call__(self, vertices, faces):
        ''' Right now only render silhouettes and depths
            ...
            faces: BN * F * 3
        '''
        # Split vertices to avoid out-of-memory
        # silhouettes = []
        # for vertice_, face_ in zip(vertices, faces):
        #     torch_mesh = Meshes(verts=vertice_.unsqueeze(0),
        #                         faces=face_.unsqueeze(0))
        #     silhouette = self.silhouette_renderer(meshes_world=torch_mesh.clone(),
        #                                         R=self.R, T=self.T)[...,3]
        #     silhouettes.append(silhouette)
        # return torch.cat(silhouettes, dim=0)
